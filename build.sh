...
# In environments without git history, fall back to a placeholder commit hash
gitCommit=$(git log --pretty=format:"%h" -1 2>/dev/null || echo "local")
# Prefer using existing public/dist by default during local builds; set USE_LOCAL_DIST=0 to force fetch
USE_LOCAL_DIST=${USE_LOCAL_DIST:-1}
...
if [ "$buildType" = "dev" ]; then
  if [ "$USE_LOCAL_DIST" = "1" ] && [ -f "public/dist/index.html" ]; then
    echo "USE_LOCAL_DIST=1, found public/dist; skip fetching frontend"
  else
    FetchWebRolling
  fi
